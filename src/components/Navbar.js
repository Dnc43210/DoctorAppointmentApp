...
export default function Navbar() {
  ...
  return (
    <Disclosure as="nav" className="bg-blue-100 fixed w-full top-0 start-0">
      {({ open }) => (
        <>
            ...
            <div className="relative flex h-16 items-center justify-between">
                  ...
                  <div className="flex space-x-4">
                    {navigation.map((item) => (
                      <Link
                        key={item.name}
                        to={item.href}
                        className="text-gray-600 hover:bg-blue-200 rounded-md px-3 py-2 text-sm font-medium duration-500"
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                ...
                <Menu as="div" className="relative ml-3">
                  <div>
                    <Menu.Button className="relative flex rounded-full bg-gray-100 text-sm text-gray-600 focus:outline-none">
                  ...
                  >
                    <Menu.Items className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                      <Menu.Item>
                        {({ active }) => (
                          <div className="hover:bg-gray-100 block px-4 py-2 text-sm text-gray-700 cursor-pointer">
                            Your Profile
                          </div>
                        )}
                      </Menu.Item>
